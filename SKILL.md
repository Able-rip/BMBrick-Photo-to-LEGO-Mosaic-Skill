---
name: photo-to-lego-mosaic-skill
description: Generate BMBrick LEGO-style mosaic preview images from local user images through the @bmbrick/photo-to-lego-mosaic-skill MCP tool. Use when a user asks an agent to make, preview, render, or convert a photo into a LEGO/brick/pixel mosaic and wants a local watermarked preview before unlocking full BMBrick project files on bmbrick.com.
---

# BMBrick Mosaic Preview

Use the `generate_lego_mosaic` MCP tool to create a local BMBrick mosaic preview from a user-provided image path.
...
2. Call `generate_lego_mosaic` with:
